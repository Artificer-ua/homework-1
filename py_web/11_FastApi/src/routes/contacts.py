...
async def search_contact(search_string: str, db: AsyncSession = Depends(get_db)):
    contact = await repositories_contacts.search_contact(search_string, db)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Item not found"
        )
...
async def search_contact_bday(db: AsyncSession = Depends(get_db)):
    contact = await repositories_contacts.search_contacts_bday(db, days=7)
    if contact is None:
        raise HTTPException(
