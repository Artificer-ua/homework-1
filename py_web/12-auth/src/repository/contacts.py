...
async def get_contacts(limit: int, offset: int, db: AsyncSession, current_user: User):
    stmt = select(Contact).filter_by(user_id=int(current_user.id)).offset(offset).limit(limit)
    contacts = await db.execute(stmt)
    return contacts.scalars().all()


async def get_contact(contact_id: int, db: AsyncSession, current_user: User):
    stmt = select(Contact).filter(and_(Contact.id == contact_id, Contact.user_id == current_user.id))
...
async def update_contact(contact_id: int, body: ContactSchema, db: AsyncSession, current_user):
    stmt = select(Contact).filter(and_(Contact.id == contact_id, Contact.user_id == current_user.id))
...
async def search_contacts_bday(current_user: User, db: AsyncSession, days):
    ...
    stmt = select(Contact).filter(and_((func.to_date(func.concat(func.to_char(Contact.bday, "DDMM"), this_year),
                                                     "DDMMYYY").between(date_from, date_to)),
                                       Contact.user_id == current_user.id))
    contact = await db.execute(stmt)
...
async def search_contact(search_string: str, db: AsyncSession, current_user: User):
    ...
    stmt = select(Contact).filter(
        and_(or_(Contact.firstname.ilike(search), Contact.lastname.ilike(search), Contact.email.ilike(search))),
        Contact.user_id == current_user.id)
    contact = await db.execute(stmt)
    return contact.scalars().all()
